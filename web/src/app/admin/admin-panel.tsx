...
const TAB_ADMIN = "administration";
...
export function AdminPanel() {
  ...
  const activeTab =
    tabParam === TAB_ADMIN && isAdmin ? TAB_ADMIN : TAB_REPORTS;

  useEffect(() => {
    if (!boot || viewerRole === null) return;
    if (tabParam === TAB_ADMIN && viewerRole !== "admin") {
      router.replace(`${pathname}?tab=${TAB_REPORTS}`);
    }
  }, [boot, tabParam, viewerRole, pathname, router]);

  const setTab = (next: typeof TAB_REPORTS | typeof TAB_ADMIN) => {
    if (next === TAB_ADMIN && !isAdmin) return;
  ...
  useEffect(() => {
    if (activeTab === TAB_ADMIN && isAdmin) {
      void loadUnfreeze();
    }
  }, [activeTab, isAdmin, loadUnfreeze]);
  ...
  const decideUnfreeze = async (userId: string, decision: "approved" | "rejected") => {
    const res = await fetch("/api/admin/unfreeze-requests", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <div>
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-xl font-semibold text-slate-100">Панель персонала</h1>
          {viewerRole === "admin" ? (
        ...
        <p className="mt-1 text-sm text-slate-400">
          Единая точка входа: жалобы и действия модерации. Раздел «Администрирование» и бан по жалобе — только у
          администраторов.
        </p>
        ...
        {activeTab === TAB_REPORTS ? (
          ...
          <section className="mt-8 space-y-4">
            <h2 className="text-lg font-medium text-slate-100">Заявки на разморозку аккаунтов</h2>
            <p className="text-sm text-slate-400">
              Пользователи с замороженным аккаунтом могут отправить заявку. Одобрение снимает заморозку; отклонение
              оставляет аккаунт замороженным.
            </p>
            {unfreezeLoading ? (
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          onClick={() => void decideUnfreeze(u.id, "approved")}
                        >
                          Одобрить
                        </button>
                        <button
                          ...
                          onClick={() => void decideUnfreeze(u.id, "rejected")}
                        >
                          Отклонить
                        </button>
                      </div>
