-- Галочка верификации канала (выдаётся модератором/админом через админку).

alter table public.users
  add column if not exists channel_verified boolean not null default false;

comment on column public.users.channel_verified is 'Верифицированный канал (галочка на сайте).';
